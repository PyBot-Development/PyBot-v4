...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, ctx, url, *, loop=None, stream=False):
        ...
        await ctx.send(embed=discord.Embed(
                    description=f'Added `{data["title"]}` to queue.', color=colours.blue
                    ), delete_after=10)
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            self.np = await self._channel.send(embed=discord.Embed(
                    description=f'Now Playing: `{source.title}`', color=colours.blue
                ))
...
class Music(commands.Cog):
    ...
    async def now_playing(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await ctx.send(
                embed=discord.Embed(
                    description='Im Not Playing Anything!.', color=colours.blue
                ), delete_after=10, )
    ...
    async def skip(self, ctx):
        ...
        vc.stop()
        await ctx.send(embed=discord.Embed(
                    description=f'{ctx.author} Skipped Current Song!', color=colours.blue
                ))
    ...
    async def volume(self, ctx, *, vol: float):
        ...
        await ctx.send(embed=discord.Embed(
                    description=f'{ctx.author} Changed Volume to `{vol}%`', color=colours.blue
                ))
    ...
    async def connect(self, ctx, *, channel: discord.VoiceChannel=None):
        ...
        await ctx.send(embed=discord.Embed(
                    description=f'Connected to `{channel}`.', color=colours.blue
                    ), delete_after=10)
def setup(bot):
